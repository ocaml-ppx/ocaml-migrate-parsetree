...
type 'a _types = 'a constraint 'a
  = <
  (*$ nl (); List.iter all_types ~f:(printf "  %-21s : _;\n") *)
  structure             : _;
  signature             : _;
  toplevel_phrase       : _;
  core_type             : _;
...
(*$ nl (); List.iter all_types ~f:(fun s ->
  printf "type 'a get_%-21s = 'x constraint 'a _types = < %s : 'x; .. >;;\n" s s) *)
...
type 'types ocaml_version =
  (module OCaml_version
    (*$ nl ();
        let w = with_then_and () in
        List.iter all_types_with_module ~f:(fun (m, s) ->
          let fq = m ^ "." ^ s in
          printf "    %s type Ast.%-31s = 'types get_%s\n" (w ()) fq s) *)
...
type ('from,'to_) migration =
  (module Migration
    (*$ nl ();
        ...
        List.iter all_types_with_module ~f:(fun (m, s) ->
          let fq = m ^ "." ^ s in
          printf "    %s type From.%-31s = 'from get_%s\n" (w ()) fq s;
          printf "    %s type   To.%-31s = 'to_  get_%s\n" (w ()) fq s) *)
...
module Id(Ast : Ast) : Migration with module From = Ast and module To = Ast = struct
  module From = Ast
  module To = Ast
  (*$ nl (); List.iter all_types ~f:(printf "let copy_%-21s x = x\n") *)
...
struct
  ...
  (*$ nl (); List.iter all_types ~f:(fun s ->
        printf "let copy_%-21s = M.copy_%s\n" s s) *)
