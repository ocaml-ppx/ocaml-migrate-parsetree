...
let ast_of_type_decls tds =
  [ { pstr_desc = Pstr_type (Nonrecursive, tds) ; pstr_loc = Location.none } ]

let ast_of_type_extension x =
  [ { pstr_desc = Pstr_typext x; pstr_loc = Location.none } ]

let ast_of_extension_constructor x =
  [ { pstr_desc = Pstr_exception x; pstr_loc = Location.none } ]

let ast_of_core_type x =
  [ { pstr_desc = Pstr_extension
          (({ txt = ""; loc = Location.none }, PTyp x), [])
    ; pstr_loc = Location.none } ]

let ast_of_expression x =
  [ { pstr_desc = Pstr_eval (x, []) ; pstr_loc = Location.none } ]

let type_decls_of_ast = function
  | [ { pstr_desc = Pstr_type (_, tds); _ } ] -> tds
  | _ -> invalid_arg "Ast_404.type_decls_of_ast"

let type_extension_of_ast = function
  | [ { pstr_desc = Pstr_typext x; _ } ] -> x
  | _ -> invalid_arg "Ast_404.extension_of_ast"

let extension_constructor_of_ast = function
  | [ { pstr_desc = Pstr_exception x; _ } ] -> x
  | _ -> invalid_arg "Ast_404.exception_of_ast"

let core_type_of_ast = function
  | [ { pstr_desc = Pstr_extension ((_, PTyp x), _); _ } ] -> x
  | _ -> invalid_arg "Ast_404.core_type_of_ast"

let expression_of_ast = function
  | [ { pstr_desc = Pstr_eval (x, _); _ } ] -> x
  | _ -> invalid_arg "Ast_404.core_type_of_ast"
