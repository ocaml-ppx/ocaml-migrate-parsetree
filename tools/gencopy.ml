...
module Main : sig end = struct
  ...
  let rec gen ty =
    if Hashtbl.mem printed ty then ()
      ...
      let params_in = List.mapi (fun i _ -> mkloc (Printf.sprintf "f%i" i) !default_loc) td.type_params in
      let params_out = List.mapi (fun i _ -> mkloc (Printf.sprintf "g%i" i) !default_loc) td.type_params in

      let env = List.map2 (fun s t -> t.id, evar s.txt) params_in td.type_params in
      ...
      let concrete e =
        let e = List.fold_right (fun x e -> lam x e) (List.map (fun x -> pvar x.txt) params_in) e in
  ...
  let main () =
    ...
    in
    let s = [ Str.module_
                (Mb.mk (mkloc "From" Location.none)
